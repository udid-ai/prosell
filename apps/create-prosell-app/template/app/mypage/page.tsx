...
import { fetchAccount, getToken, won, type AccountField } from "@/lib/prosell";
...
export default async function MyPage() {
  ...
  const account: Row[] = [
    ["아이디", v(o.uid)],
    ["회원등급", o.level != null ? `LV.${o.level}` : null],
    ["상태", active ? "정상" : "이용제한"],
    ["가입일", fmtDate(o.dt)],
  ...
  const profile: Row[] = [
    ...
    ["휴대폰", o.hp ? `${o.hp} (수신 ${yn(o.hp_receive)})` : null],
    ["일반전화", v(o.tel)],
  ];
  const address: Row[] = [
    ["우편번호", v(o.zipcode)],
  ...
  const extra: Row[] = [
    ["거래은행", i.bank ? String(i.bank) : null],
    ["계좌번호", v(i.banknum)],
    ["예금주", v(i.bankholder)],
    ["관심분야", v(i.interest)],
    ["자기소개", v(i.profile)],
  ...
  return (
    <main className={cardCls}>
      <div className="flex items-center justify-between">
        <h1 className="text-xl">내 정보</h1>
        <Link href="/mypage/edit" className="rounded-sm border border-line px-3 py-1.5 text-[13px] text-text hover:bg-line">정보 수정</Link>
      </div>
