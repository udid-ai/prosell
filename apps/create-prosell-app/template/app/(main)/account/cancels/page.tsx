...
import { getToken, fetchMemberCancels, imgUrl, won, type MemberCancel, type MemberOrderItem } from "@/lib/prosell";
...
// can_state: 0 취소접수(환불전) · 1 처리중 · 2 취소완료(환불완료)
...
export default async function CancelsPage({ searchParams }: { searchParams: Promise<{ page?: string; months?: string; start?: string; end?: string }> }) {
  const token = await getToken();
  if (!token) {
    return (
      <div className={cardCls}>
        <h1 className="text-xl">취소 내역</h1>
  ...
  const limit = 10;
  const { cancels, total_count } = await fetchMemberCancels(token, custom ? { page, limit, start: sp.start, end: sp.end } : { page, months, limit });
    ...
    <div className="space-y-4">
      <div className="flex min-h-10 flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-1 rounded-md border border-line bg-card p-1">
          {PERIODS.map((m) => (
            <Link key={m} href={`/account/cancels?months=${m}`}
          ...
          <OrderPeriodPicker start={sp.start} end={sp.end} active={custom} basePath="/account/cancels" />
        </div>
      </div>
      ...
      {cancels.length === 0 ? (
        <div className="rounded-md border border-line bg-card p-12 text-center text-sub">
          {custom ? "해당 기간의 취소 내역이 없습니다." : `최근 ${months}개월 취소 내역이 없습니다.`}
        </div>
      ) : (
        <ul className="space-y-4">
          {cancels.map((c) => <CancelCard key={c.cancel.cno} c={c} />)}
        </ul>
      )}
      ...
      {totalPages > 1 && (
        <nav className="flex items-center justify-center gap-1 pt-2">
          {Array.from({ length: totalPages }).map((_, i) => {
            const p = i + 1;
            return (
              <Link key={p} href={`/account/cancels?${periodQS}&page=${p}`}
...
function CancelCard({ c }: { c: MemberCancel }) {
  const cn = c.cancel;
    ...
    <li className="overflow-hidden rounded-2xl border border-line bg-card">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line bg-surface/60 px-5 py-3">
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <Link href={`/account/cancels/${cn.cno}`} className="text-[13px] font-semibold text-text hover:text-accent">취소번호 {cn.cno}</Link>
        </div>
        <span className="text-[13px] text-sub">{formatDateTime(cn.can_dt)}</span>
      </div>

      {/* 취소 상품 — 주문내역 UI 와 동일(상태 뱃지·가격셀·상세내역 액션 컬럼) */}
      <ul className="divide-y divide-line">
        {lines.map((ln) => {
          ...
          const price = p.can_pro_amount_price ?? p.pro_amount_price;
          return (
            <li key={p.prno} className="flex flex-col px-0 sm:flex-row sm:items-stretch">
              ...
              <div className="flex min-w-0 items-stretch sm:flex-1">
                ...
                <div className="min-w-0 flex-1 py-4 pr-4">
                  <Link href={`/account/cancels/${cn.cno}`} className="line-clamp-2 w-fit text-sm font-semibold text-text hover:text-accent">
              ...
              <div className="flex w-full flex-row flex-wrap items-center justify-center gap-1.5 border-t border-line py-3 sm:w-28 sm:shrink-0 sm:flex-col sm:items-center sm:justify-center sm:border-t-0 sm:border-l sm:border-line sm:py-4">
                <Link href={`/account/cancels/${cn.cno}`}
