...
import { getToken, fetchWishlist, fetchProducts, priceOf, thumbOf, optionOf, won, type ProductItem } from "@/lib/prosell";
...
export default async function WishlistPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const { items, total_count, wishlist_day } = await fetchWishlist(token, page, limit);

  // 상품 카드 데이터는 목록 API(id 필터)로 일괄 조회 — 판매중(onoff=1)만.
  const ids = items.map((w) => w.products_id);
  ...
  // 위시리스트 담은 순서(dt desc) 유지 + 존재하는 상품만. dt 는 위시리스트 담은 날짜.
  const rows = items
    .map((w) => {
      const p = byId.get(w.products_id);
      if (!p || !p.origin?.id) return null;
      const { price, base } = priceOf(p);
      return {
        wid: w.id, products_id: w.products_id, dt: w.dt,
    ...
    <div className="space-y-4">
      ...
      <div className="flex min-h-10 flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          {wishlist_day > 0 && (
            <span className="text-[12px] text-sub">보관함에 담긴 상품은 담은 날로부터 {wishlist_day}일 경과 후 삭제됩니다.</span>
          )}
          {total_count > 0 && <ListClearButton url="/api/account/wishlist?all=1" message="보관함의 상품을 모두 삭제하시겠습니까?" />}
        </div>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <section className="overflow-hidden rounded-2xl border border-line bg-card">
          <ul className="divide-y divide-line">
            {rows.map((r) => (
              <li key={r.wid} className="flex flex-col px-0 sm:flex-row sm:items-stretch">
                ...
                {/* 액션 — 바로구매·장바구니·삭제 (모바일 하단 / sm 우측 세로열) */}
                <div className="flex w-full items-center justify-center border-t border-line px-4 py-3 sm:w-32 sm:shrink-0 sm:border-l sm:border-t-0 sm:py-4">
                  <ProductRowActions productsId={r.products_id} productId={r.productId} title={r.title} price={r.price} soldout={r.soldout} canOrder={r.viewOpen && r.orderOpen} removeBase="/api/account/wishlist" />
                </div>
              </li>
            ))}
          </ul>
      ...
      {totalPages > 1 && (
        <nav className="flex items-center justify-center gap-1 pt-2">
          {Array.from({ length: totalPages }).map((_, i) => {
            const p = i + 1;
            return (
              <Link key={p} href={`/account/wishlist?page=${p}`}
