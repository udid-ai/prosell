...
- **장바구니/주문** — 서버 장바구니(그룹·쿠폰·적립금), 바로구매, oid 세션 체크아웃(PG/무통장/전액포인트),
  국내/해외 배송지, 주문완료·영수증. 비회원 주문 정책(order_guest: 불가/바로/로그인 경유) 게이트.
...
- **CS** — 통합/개별 게시판(board_type) 인식. 공지사항·FAQ(카테고리 탭·제목 검색·페이징),
  1:1문의·상품문의(위지윅 에디터[글자 크기·색상·이미지·동영상], 파일 첨부, 문의대상 상품 선택[개별],
  비밀글·비밀번호 열람, 답변 알림), 약관·정보 페이지.
