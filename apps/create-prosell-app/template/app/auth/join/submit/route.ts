...
import { signup, loginMember, clientIpFromHeaders, AT, RT, atCookieMaxAge, type SignupInput } from "@/lib/prosell";

export const dynamic = "force-dynamic";

// JoinForm(클라이언트 위저드)용 가입 프록시 — client_id 를 서버에만 두고 가입을 중계한다.
export async function POST(req: NextRequest) {
  const b = (await req.json().catch(() => ({}))) as Partial<SignupInput> & Record<string, unknown>;
  ...
  res.cookies.set(AT, login.access_token!, { httpOnly: true, path: "/", sameSite: "lax", secure, maxAge: atCookieMaxAge(login.expires_in) });
  if (login.refresh_token) {
    res.cookies.set(RT, login.refresh_token, { httpOnly: true, path: "/", sameSite: "lax", secure, maxAge: login.refresh_token_expires_in || 2592000 });
  }
  return res;
}
