...
// 반품 회수 운송장 등록/수정 — 레거시 refund/parcel 모달. 회수중(ref_state==20)·택배건에서만 노출(부모가 판정).
// 회수 택배사는 표시(읽기전용), 운송장 번호만 입력. PUT /api/order/refund/parcel { rno, ref_ret_num }
export default function RefundParcelButton({ rno, courier, initial }: { rno: number; courier?: string | null; initial?: string | null }) {
  ...
  return (
    <>
      ...
      {open && <RefundParcelModal rno={rno} courier={courier} initial={initial} onClose={() => setOpen(false)} />}
    </>
  );
}

function RefundParcelModal({ rno, courier, initial, onClose }: { rno: number; courier?: string | null; initial?: string | null; onClose: () => void }) {
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      const res = await fetch("/api/order/refund/parcel", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rno: String(rno), ref_ret_num: v }) });
    ...
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-line bg-card p-6 shadow-xl" style={{ maxHeight: "90vh" }}>
        ...
        <div className="space-y-3">
          ...
          <p className="text-[12px] leading-relaxed text-sub">반품 상품을 위 택배사로 발송하신 뒤 운송장 번호를 등록해 주세요. 검수 완료 후 환불이 진행됩니다.</p>
